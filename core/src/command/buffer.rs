...
impl Usage for OneTimeUse {
    fn flags() -> ash::vk::CommandBufferUsageFlags {
        ash::vk::COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT
    }
}
...
impl Usage for MultiUse<Simultaneous> {
    fn flags() -> ash::vk::CommandBufferUsageFlags {
        ash::vk::COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT
    }
}
    ...
    L: Level,
{
    ...
    pub(crate) unsafe fn begin<U: Usage, T>(self, tracker: T) -> Result<Buffer<C, Recording<U, T>, R, L>, OomError> {
        unimplemented!()
    }
}
...
impl<C, U, T, R> Buffer<C, Executable<U, T>, R, Primary> {
    ...
    pub(crate) unsafe fn submit_once(self) -> (Submit<C, T>, Buffer<C, Pending<Invalid>, R, Primary>) {
        unimplemented!()
    }
}
...
impl<C, S, T, R> Buffer<C, Executable<MultiUse<S>, T>, R, Primary> {
    ...
    pub(crate) unsafe fn submit(self) -> (Submit<C>, Buffer<C, Pending<Executable<MultiUse<S>, T>>, R, Primary>) {
        unimplemented!()
    }
}
...
impl<C, S, R, L> Buffer<C, S, R, L> {
    ...
    pub(crate) unsafe fn from_raw(fp: Arc<ash::vk::DeviceFnV1_0>, raw: ash::vk::CommandBuffer, family: FamilyId<C>, state: S) -> Self {
        Buffer {
            fp,
            raw,
            family,
