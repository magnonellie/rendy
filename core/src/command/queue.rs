...
use std::{collections::LinkedList, fmt::Debug, marker::PhantomData, ops::Add, ptr::null, sync::Arc};
use ash;
    ...
    C: Copy + Debug,
{
        ...
        F: FnMut(T),
    {
        ...
        let submissions = submissions.into_iter().map(|submission| {
            let wait_count = submission.wait.into_iter().fold(0, |acc, (semaphore, stage)| {
    ...
    pub unsafe fn from_raw(fp: Arc<ash::vk::DeviceFnV1_0>, raw: ash::vk::Queue, id: QueueId<C>) -> Self {
        Queue {
            fp,
            raw,
            id,
    ...
    pub(crate) unsafe fn complete(&mut self, device: ash::vk::Device, fence: ReadyFence<C>) -> UnarmedFence {
