...
impl Usage for ResetBuffer {
    fn flags() -> ash::vk::CommandPoolCreateFlags {
        ash::vk::COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT
    }
}
    ...
    C: Copy,
{
    ...
    pub(crate) unsafe fn from_raw(fp: Arc<ash::vk::DeviceFnV1_0>, raw: ash::vk::CommandPool, device: ash::vk::Device, family: FamilyId<C>) -> Self {
        Pool {
            fp,
            device,
            raw,
    ...
    pub fn allocate<L: Level>(&mut self, count: u32) -> Result<impl Iterator<Item = Buffer<C, Initial, R, L>>, OomError> {
        unsafe {
